...
model_ready = False  # track availability
...
def load_models(base_model: str, lora_dir: str | None):
    ...
    try:
        ...
        model = base
        model_ready = True
        app.logger.info("Model loaded and ready.")
    except Exception as e:
        log_ex(e)
        model_ready = False
        app.logger.warning("Model failed to load. Falling back to toy replies.")
...
def toy_reply(turns):
    ...
    for t in reversed(turns):
        if t.get("role") == "user":
            ...
            break
    if any(w in last_user.lower() for w in ["hi", "hello", "hey"]):
        return "Hi! The model isn’t ready yet, but I’m here. 🙂"
    return "Model not ready yet; using fallback reply."
...
def summarize_dialog(turns, max_new_tokens=150):
    dialog_text = to_dialog_text(turns)
    if not model_ready:
        return "(Model not ready) Example summary: Two people discuss a topic and agree on next steps."
...
def api_summarize():
    try:
        data = request.get_json(force=True) or {}
        turns = data.get("turns", [])
        summary = summarize_dialog(turns)
